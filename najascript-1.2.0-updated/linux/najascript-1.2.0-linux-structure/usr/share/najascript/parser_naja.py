...
class Parser:
    ...
    def primary_expression(self):
        ...
        if token.type == TokenType.AWAIT:
            ...
            # Se for super() sem método específico, trata como super.constructor()
                ...
                return SuperExpression("constructor", arguments)
    ...
    def import_statement(self):
        """
        import_statement : IMPORT STRING_LIT [AS IDENTIFIER] ';'
                         | IMPORT IDENTIFIER FROM STRING_LIT ';'                     // default import
                         | IMPORT '{' import_item_list '}' FROM STRING_LIT ';'       // named imports
                         | IMPORT '*' AS IDENTIFIER FROM STRING_LIT ';'              // namespace import
        ...
        if self.current_token.type == TokenType.MULTIPLY:
            ...
            namespace_name = self.current_token.value
            self.eat(TokenType.IDENTIFIER)
            ...
            return ImportStatement(module_name, is_import_all=True, module_alias=namespace_name)
        
        # Case 2: import { item1, item2 as alias, ... } from "module"
            ...
            if self.current_token.type == TokenType.IDENTIFIER:
                ...
                if self.current_token.type == TokenType.AS:
                    self.eat(TokenType.AS)
                    item_alias = self.current_token.value
                    self.eat(TokenType.IDENTIFIER)
                
                import_items.append(ImportItem(item_name, item_alias))
            ...
            return ImportStatement(module_name, import_items=import_items)
        
        # Case 3: import defaultExport from "module"  
        elif self.current_token.type == TokenType.IDENTIFIER:
            default_name = self.current_token.value
            self.eat(TokenType.IDENTIFIER)
            self.eat(TokenType.FROM)
            module_name = self.current_token.value
            self.eat(TokenType.STRING_LIT)
            ...
            return ImportStatement(module_name, import_items=[ImportItem("default", default_name)], is_default_import=True)
        
        # Case 4: import "module" [as alias]
        elif self.current_token.type == TokenType.STRING_LIT:
            module_name = self.current_token.value
            self.eat(TokenType.STRING_LIT)
            ...
            if self.current_token.type == TokenType.AS:
                self.eat(TokenType.AS)
                module_alias = self.current_token.value
                self.eat(TokenType.IDENTIFIER)
            
            if self.current_token.type == TokenType.SEMICOLON:
                self.eat(TokenType.SEMICOLON)
                
            return ImportStatement(module_name, module_alias=module_alias)
        
        else:
            self.error(f"Sintaxe de import inválida. Token inesperado: {self.current_token.type}")
    ...
    def export_statement(self):
        ...
        export_statement : EXPORT function_declaration
                         ...
                         | EXPORT IDENTIFIER ';'
                         | EXPORT DEFAULT (function_declaration | expression) ';'
                         | EXPORT '{' export_item_list '}' [FROM STRING_LIT] ';'
                         | EXPORT '*' FROM STRING_LIT ';'
                         | EXPORT '*' AS IDENTIFIER FROM STRING_LIT ';'
        ...
        if self.current_token.type == TokenType.DEFAULT:
            ...
            # Pode ser uma função ou expressão
            if self.current_token.type == TokenType.FUN:
                # export default function
                func_decl = self.function_declaration(exported=True)
                return ExportStatement(func_decl.name, is_default=True)
            else:
                # export default expression
                expr = self.expression()
                if self.current_token.type == TokenType.SEMICOLON:
                    self.eat(TokenType.SEMICOLON)
                return ExportStatement(expr, is_default=True)
        ...
        # Case 2: export * from "module" ou export * as name from "module"
            ...
            if alias:
                return ExportStatement(alias, from_module=module_name, is_all_export=True)
            else:
                return ExportStatement(from_module=module_name, is_all_export=True)
        ...
        # Case 3: export { item1, item2 as alias, ... } [from "module"]
            ...
            if self.current_token.type == TokenType.IDENTIFIER:
                ...
                export_items.append(ExportItem(item_name, item_alias))
            ...
            # Opcional: from "module" para re-export
            from_module = None
            if self.current_token.type == TokenType.FROM:
                self.eat(TokenType.FROM)
                from_module = self.current_token.value
                self.eat(TokenType.STRING_LIT)
            
            if self.current_token.type == TokenType.SEMICOLON:
                self.eat(TokenType.SEMICOLON)
            
            return ExportStatement(export_items=export_items, from_module=from_module)
        
        # Case 4: export function/var/const declarations
        elif self.current_token.type == TokenType.FUN:
            return self.function_declaration(exported=True)
        elif self.current_token.type == TokenType.CONST:
            # export const tipo nome = valor;
            is_const = True
            self.eat(TokenType.CONST)
            ...
            self.error(f"Sintaxe de export inválida. Token inesperado: {self.current_token.type}") 